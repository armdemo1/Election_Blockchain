...
function PollResult() {
  ...
  function renderCloseButton(poll, pollIdx) {
    if (!poll.isRunning) {
      return (
        <button className="boxLight notification"
        >
          Poll Closed
        </button>
      );
  ...
  function renderResult(options, voteCounts) {
    ...
    return state.allPolls.map((poll, idx) => {
      return (
        <div className="section" key={idx}>
          ...
          { renderResult(poll.options, poll.voteCounts) }
  ...
  return (
    <main className="main">
      <h2 className="pageHeader">Current Polls Result</h2>
